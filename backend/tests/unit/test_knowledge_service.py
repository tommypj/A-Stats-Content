...
class TestKnowledgeServiceProcessing:
    ...
    async def test_process_document_failure_updates_error(self, service, mock_dependencies):
        ...
        mock_dependencies['document_processor'].process_file.side_effect = Exception("Processing failed")
...
class TestKnowledgeServiceQuery:
    ...
    async def test_query_knowledge_success(self, service, mock_dependencies):
        ...
        mock_dependencies['chroma_adapter'].query.return_value = {
            ...
            'documents': [[
                ...
                'It focuses on changing thought patterns.'
            ]],
            'distances': [[0.1, 0.2]],
            'metadatas': [[
                {'source': 'therapy.pdf', 'page': 1},
                {'source': 'therapy.pdf', 'page': 2}
            ]]
        }

        # Mock LLM response
        mock_dependencies['llm_service'].generate_answer.return_value = \
            "Cognitive Behavioral Therapy (CBT) is a form of psychotherapy that focuses on changing thought patterns."
    ...
    async def test_query_knowledge_no_results(self, service, mock_dependencies):
        ...
        assert 'answer' in result
        assert "no information" in result['answer'].lower() or "not found" in result['answer'].lower()
...
class TestKnowledgeServiceStats:
    ...
    async def test_get_user_stats(self, service, mock_dependencies):
        ...
        mock_dependencies['db_session'].execute.return_value.scalars.return_value.all.return_value = mock_sources

        # Mock ChromaDB stats
        mock_dependencies['chroma_adapter'].get_stats.return_value = {
            'total_documents': 150
        }
