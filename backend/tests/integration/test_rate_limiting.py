...
class TestRateLimitingLogin:
    ...
    async def test_login_rate_limit_exceeded(self, async_client: AsyncClient, test_user: User):
        ...
        response = await async_client.post(
            "/api/v1/auth/login", json={"email": "another@example.com", "password": "wrongpassword"}
        )
        assert response.status_code == 429

    async def test_login_rate_limit_with_valid_credentials(
...
class TestRateLimitingPasswordReset:
    ...
    async def test_password_reset_rate_limit_exceeded(self, async_client: AsyncClient):
        ...
        for i in range(3):
            response = await async_client.post(
                "/api/v1/auth/password/reset-request", json={"email": f"user{i}@example.com"}
            )
        ...
        # 4th attempt should be rate limited
        response = await async_client.post(
            "/api/v1/auth/password/reset-request", json={"email": "user4@example.com"}
        )
        assert response.status_code == 429
...
class TestRateLimitingHeaders:
    ...
    async def test_rate_limit_headers_present(self, async_client: AsyncClient):
        ...
        response = await async_client.post(
            "/api/v1/auth/password/reset-request", json={"email": "test@example.com"}
        )
        assert response.status_code == 202
    ...
    async def test_rate_limit_429_response_format(self, async_client: AsyncClient):
        ...
        for i in range(3):
            await async_client.post(
                "/api/v1/auth/password/reset-request", json={"email": f"test{i}@example.com"}
            )

        # Next request should be rate limited
        response = await async_client.post(
            "/api/v1/auth/password/reset-request", json={"email": "test4@example.com"}
        )
        assert response.status_code == 429
...
class TestRateLimitingDifferentEndpoints:
    ...
    async def test_different_endpoints_have_independent_limits(
        ...
        response = await async_client.post(
            "/api/v1/auth/login", json={"email": "user6@example.com", "password": "wrongpass"}
        )
        assert response.status_code == 429
