"""
Team invitation background service.
"""
...
from infrastructure.database.models import TeamInvitation, InvitationStatus
...
async def expire_old_invitations(db: AsyncSession) -> int:
    """
    Mark expired team invitations as EXPIRED.
    ...
    if expired_count > 0:
        await db.commit()
        logger.info(f"Marked {expired_count} team invitations as expired")
...
async def cleanup_old_invitations(db: AsyncSession, days_old: int = 30) -> int:
    ...
    if deleted_count > 0:
        await db.commit()
        logger.info(f"Deleted {deleted_count} old team invitations")
