...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "goal_type", sa.String(50), nullable=False
        ),  # page_visit, form_submit, purchase, custom
        sa.Column(
            "goal_config", sa.JSON, nullable=True
        ),  # target_url_pattern, event_name, revenue_value
    ...
    op.create_table(
        ...
        sa.Column(
            "goal_id",
            UUID(as_uuid=False),
            sa.ForeignKey("conversion_goals.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        ),
    ...
    op.create_index(
        "ix_content_conversions_article_date", "content_conversions", ["article_id", "date"]
    )
