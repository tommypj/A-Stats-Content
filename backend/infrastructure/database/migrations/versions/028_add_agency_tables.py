...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "agency_id",
            UUID(as_uuid=False),
            sa.ForeignKey("agency_profiles.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_client_workspaces_agency_client", "client_workspaces", ["agency_id", "client_name"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "template_config", sa.JSON, nullable=False
        ),  # {sections, branding, metrics_to_include}
        sa.Column(
            "created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False
        ),
        sa.Column(
    ...
    op.create_table(
        ...
        sa.Column(
            "client_workspace_id",
            UUID(as_uuid=False),
            sa.ForeignKey("client_workspaces.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_generated_reports_agency_generated", "generated_reports", ["agency_id", "generated_at"]
    )
    op.create_index(
        "ix_generated_reports_workspace_generated",
        "generated_reports",
        ["client_workspace_id", "generated_at"],
    )
