...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("article_id", UUID(as_uuid=False), sa.ForeignKey("articles.id", ondelete="CASCADE"), nullable=False, index=True),
        ...
        sa.Column("score_breakdown", sa.JSON, nullable=True),  # {structure_score, faq_score, entity_score, conciseness_score, schema_score, citation_readiness}
    ...
    op.create_table(
        ...
        sa.Column("user_id", UUID(as_uuid=False), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("project_id", UUID(as_uuid=False), sa.ForeignKey("projects.id", ondelete="CASCADE"), nullable=True, index=True),
        sa.Column("source", sa.String(50), nullable=False),  # chatgpt, perplexity, gemini, bing_copilot
