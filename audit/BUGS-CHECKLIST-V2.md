...
| PROJ-37 | 🟠 | [x] | Ownership transfer doesn't prevent attacker from locking out original owner afterward | projects.py | 703-741 |
...
| GEN-39 | 🟢 | [x] | `meta_description` stored without length validation — max 160 chars not enforced | articles.py | 215-268 |
| GEN-40 | 🟠 | [x] | Log truncation on JSON parse failure not applied consistently | articles.py | 751-755 |
...
| BILL-18 | 🟠 | [x] | Project webhook handler missing `with_for_update()` — race condition on concurrent webhooks | billing.py | 300 |
| BILL-19 | 🟠 | [x] | Webhook idempotency degrades silently without Redis — duplicate events can double-charge | billing.py | 485-487 |
...
| KV-12 | 🟡 | [x] | No per-source rate limiting on knowledge query endpoint | knowledge.py | 488-517 |
| ANA-35 | 🟢 | [x] | `GSC_DATA_LAG_DAYS` not communicated to user — users confused by old data | analytics.py | 81-82 |
| ANA-36 | 🟢 | [x] | `goal_config` accepted without schema validation per `goal_type` | analytics.py | 1877-1948 |
| KV-13 | 🟢 | [x] | File deletion success not logged — hard to audit file operations | knowledge.py | 468-478 |
| ANA-37 | 🟢 | [x] | `_normalize_url` has minimal comments — hard to maintain | revenue_attribution.py | 50-68 |
| ANA-38 | 🟢 | [x] | AEO suggestion JSON parsing uses fragile string manipulation instead of regex | aeo_scoring.py | 349-362 |
| KV-14 | 🟢 | [x] | File-size error message uses float division — could show unexpected values | knowledge.py | 206-210 |
| ANA-39 | 🟢 | [x] | `get_content_health_score` recomputes from scratch every call — no caching | content_decay.py | 360-434 |
...
| SM-27 | 🟢 | [x] | Facebook photo upload doesn't validate file size before attempting upload | facebook_adapter.py | 548 |
| SM-28 | 🟡 | [x] | LinkedIn presigned upload URL not validated to be a LinkedIn-controlled domain | linkedin_adapter.py | 541-545 |
| SM-29 | 🔴 | [x] | Scheduler: single-process mutex missing to prevent concurrent `process_due_posts()` | social_scheduler.py | 67-115 |
| SM-30 | 🟡 | [x] | `retry_after` value from rate limit error not stored — next retry ignores it | social_scheduler.py | 306-330 |
| SM-31 | 🟠 | [x] | Credential decryption failure doesn't mark account inactive or notify user | social_scheduler.py | 227-235 |
| SM-32 | 🟠 | [x] | Token refresh failure disables account but never sends email notification (TODO) | social_scheduler.py | 246-261 |
...
| SM-39 | 🟢 | [x] | Profile picture URLs from Facebook not validated to CDN domain before storage | social.py | 363, 402 |
...
| IMG-23 | 🟢 | [x] | `int(content_length)` conversion not wrapped in try/except — malformed header crashes | image_storage.py | 421-423 |
| IMG-24 | 🟢 | [x] | Orphaned file cleanup after failed DB commit has no retry logic | images.py | 129-137 |
...
| IMG-27 | 🟡 | [x] | `set_featured_image` — image and article validated per-project but not related to each other (by design) | images.py | 458-508 |
| IMG-28 | 🟢 | [x] | S3 key generation doesn't re-sanitize filename for path traversal | image_storage.py | 262 |
| IMG-29 | 🟠 | [x] | S3 key extraction from URL is fragile — malformed URL could extract wrong key | image_storage.py | 336-343 |
...
| IMG-33 | 🟢 | [x] | Image generation semaphore hardcoded to 3 — should be configurable | images.py | 40 |
...
| ID | Sev | Status | Description | File | Line |
|----|-----|--------|-------------|------|------|
| AGY-14 | 🟡 | [x] | Portal timeout can leak workspace existence via timing differences | agency.py | 916-1022 |
...
| ID | Sev | Status | Description | File | Line |
|----|-----|--------|-------------|------|------|
| CROSS-01 | 🟡 | [x] | Missing rate limiting on bulk admin operations (DoS vector) | Multiple files | — |
| CROSS-02 | 🟠 | [x] | Inconsistent audit logging — template CRUD operations not audited | bulk.py, admin_alerts.py | — |
| CROSS-03 | 🟡 | [x] | Global max-results cap missing — all list endpoints honor page_size ≤ 100 but no total cap | Multiple files | — |
...
| ID | Sev | Status | Description | File | Line |
|----|-----|--------|-------------|------|------|
| DB-01 | 🟠 | [x] | Migration 032 uses `String(36)` for FK instead of `UUID(as_uuid=False)` — type inconsistency | 032_add_knowledge_project_id.py | 26 |
| DB-02 | 🟠 | [x] | GSC `access_token` and `refresh_token` stored as plain Text — not encrypted at rest | analytics.py model | 60-61 |
...
| DB-05 | 🟡 | [x] | `Project.slug` is globally unique — should be unique per owner (owner_id, slug) | project.py model | 50-51 |
| DB-06 | 🟡 | [x] | No index on `ProjectMember.deleted_at` — soft-delete queries do full table scan | project.py model | — |
| DB-07 | 🟡 | [x] | GSC OAuth tokens not rotated — stale tokens usable indefinitely if DB is compromised | analytics.py model | 60-64 |
...
| INFRA-01 | 🟠 | [x] | Password hash column `String(255)` — use `Text` for future-proofing | user.py model | 66 |
| INFRA-02 | 🟡 | [x] | Redis connection pool not configured for rate limiter — each check may create new connection | main.py | 98 |
| INFRA-03 | 🟢 | [x] | CSP header not yet added — TODO comment exists | main.py | 256 |
| INFRA-06 | 🟡 | [x] | Global exception handler logs raw `str(exc)` — may contain sensitive data (DB strings, paths) | main.py | 196-206 |
| INFRA-11 | 🟢 | [x] | `/logout` endpoint has no rate limit decorator | auth.py | 584 |
| INFRA-16 | 🟡 | [x] | DB connection doesn't enforce SSL in production | connection.py | 17-23 |
| INFRA-17 | 🟢 | [x] | `X-Request-ID` not propagated to outbound API calls (Anthropic, email, etc.) | main.py | 235-251 |
| INFRA-18 | 🟢 | [x] | `get_db_context()` has no timeout — could exhaust connection pool | connection.py | 50-61 |

### Logging & Config Issues

| ID | Sev | Status | Description | File | Line |
|----|-----|--------|-------------|------|------|
| LOGGING-01 | 🟢 | [x] | `JSONFormatter` doesn't handle arbitrary nested extra fields | logging_config.py | 25-27 |
| LOGGING-02 | 🟢 | [x] | Uvicorn access logs silenced at WARNING level — loses request visibility | logging_config.py | 62 |
| CONFIG-01 | 🟢 | [x] | Invalid JSON in `CORS_ORIGINS` silently falls back to comma-split — no error raised | settings.py | 82-92 |
...
| RATE-LIMIT-02 | 🟢 | [x] | `X-Forwarded-For` with private IP (127.0.0.1) not rejected — rate limit bucket spoofing | rate_limit.py | 45-66 |
...
| FE-AUTH-09 | 🟠 | [x] | Multiple Enter presses can submit login form multiple times | login/page.tsx | 139-146 |
| FE-AUTH-10 | 🟠 | [x] | Email field registered in form but disabled — if enabled via DevTools, change is silently dropped | settings/page.tsx | 100-107 |
...
| FE-AUTH-28 | 🟢 | [x] | Notification dropdown stays open after notification click navigates away | layout.tsx | 211-214 |
...
| ID | Sev | Status | Description | File | Line |
|----|-----|--------|-------------|------|------|
| FE-CONTENT-01 | 🟠 | [x] | Article generation polling race — state update on unmounted component (memory leak) | articles/new/page.tsx | 98-126 |
| FE-CONTENT-02 | 🟡 | [x] | Keyword field accepts blank/whitespace — no client-side validation | articles/page.tsx | 416 |
| FE-CONTENT-03 | 🟡 | [x] | Outline word count `min={50}` can be bypassed — no max or trim on headings | outlines/[id]/page.tsx | 361-370 |
| FE-CONTENT-04 | 🟡 | [x] | Auto-save snapshot ref not reset on unmount — stale snapshot after re-open | articles/[id]/page.tsx | 985-1024 |
...
| FE-CONTENT-07 | 🟡 | [x] | Bulk selection state cleared on every keystroke in search — not debounced | articles/page.tsx | 412-418 |
| FE-CONTENT-08 | 🟠 | [x] | `handleCreate` button — no unique request ID, duplicate job submission possible | bulk/page.tsx | 97-123 |
...
| FE-CONTENT-10 | 🟡 | [x] | Custom instructions textarea — no warning at 80% char limit, paste can overflow | articles/new/page.tsx | 299-309 |
...
| FE-CONTENT-13 | 🟠 | [x] | Race condition: auto-save can fire concurrently with revision restore | articles/[id]/page.tsx | 1083-1110 |
| FE-CONTENT-14 | 🟡 | [x] | `restoreRevision` result not null-checked before destructuring | articles/[id]/page.tsx | 1088 |
| FE-CONTENT-15 | 🟡 | [x] | Inconsistent error message wording across articles/outlines/bulk pages | Multiple | — |
...
| FE-CONTENT-18 | 🟡 | [x] | Bulk job items list renders all items without pagination — slow at 500+ items | bulk/jobs/[id]/page.tsx | 189-231 |
| FE-CONTENT-19 | 🟡 | [x] | Word count duplicated in editor and `seo-score.ts` — two implementations diverge | articles/[id]/page.tsx | 65-66 |
...
| FE-CONTENT-26 | 🟡 | [x] | Retry toast may not preserve POST request config properly | api.ts | 211-235 |
| FE-CONTENT-27 | 🟡 | [x] | Manual article creation shows generic field error, no per-field highlighting | articles/new/page.tsx | 133-137 |
| FE-CONTENT-28 | 🟠 | [x] | `setInterval` polling not guaranteed to clear on unmount — memory leak | articles/new/page.tsx | 95-126 |
| FE-CONTENT-29 | 🟡 | [x] | Article slug passed to SerpPreview without validation — undefined breaks preview | articles/[id]/page.tsx | 1569 |
| FE-CONTENT-30 | 🟢 | [x] | Pagination and filter state not persisted in URL — lost on navigation | articles/page.tsx | 69-76 |
...
| FE-IMAGES-03 | 🟢 | [x] | Client-side filtering bypasses pagination — all matches shown without paging | images/page.tsx | 129-152 |
| FE-IMAGES-04 | 🟢 | [x] | No loading state on copy/download/send image operations | images/page.tsx | 523-569 |
...
| FE-ADMIN-02 | 🟢 | [x] | Stats API null/undefined fields crash PieChart rendering | admin/page.tsx | 74-81 |

### Portal

| ID | Sev | Status | Description | File | Line |
|----|-----|--------|-------------|------|------|
| FE-PORTAL-01 | 🟢 | [x] | `brand_colors` hex not validated before use in CSS — invalid value breaks styling | portal/[token]/page.tsx | 133-137 |
| FE-PORTAL-02 | 🟡 | [x] | Portal data fetch has no timeout — spinner hangs indefinitely | portal/[token]/page.tsx | 108-125 |
| FE-PORTAL-03 | 🟢 | [x] | `footer_text` rendered as raw HTML — XSS if agency inputs malicious HTML | portal/[token]/page.tsx | 387-388 |

### Cross-Cutting Frontend

| ID | Sev | Status | Description | File | Line |
|----|-----|--------|-------------|------|------|
| FE-MISC-01 | 🟡 | [x] | Inconsistent error handling — some use toast, some only set state | Multiple | — |
...
| FE-MISC-07 | 🟢 | [x] | Missing helpful empty states in some pages (table shows blank instead of CTA) | Multiple | — |
